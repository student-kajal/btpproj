...
      // 2. Header detection and index extraction (as per your logic)
      let dataStartIndex = -1, headers = [];
      for (let i = 0; i < Math.min(jsonData.length, 10); i++) {
        const row = jsonData[i];
        if (row && Array.isArray(row)) {
          const headerIndex = row.findIndex(cell => cell && typeof cell === 'string' && (cell.includes('SL NO') || cell.includes('Student ID')));
          if (headerIndex !== -1) { dataStartIndex = i; headers = row; break; }
        }
      }
      if (dataStartIndex === -1) return { success: 0, errors: ['Header row not found'], duplicates: 0 };
...
        email: headers.findIndex(h => h && (h.toString().includes('Email') || h.toString().includes('E-mail'))),
...
        if (!email || email === '') email = `${studentId.toString().toLowerCase().replace(/[^a-z0-9]/g, '')}@nsut.ac.in`;
        const cleanEmail = email.toString().toLowerCase().trim();
...
      // 4. **Bulk Password Hashing — CORE FIX**
      for (let user of studentsToCreate) {
...
        const row = jsonData[i];
        if (row && Array.isArray(row)) {
          const headerIndex = row.findIndex(cell => cell && typeof cell === 'string' && (cell.includes('Staff ID') || cell.includes('Employee ID')));
          if (headerIndex !== -1) { dataStartIndex = i; headers = row; break; }
        }
      }
      if (dataStartIndex === -1) return { success: 0, errors: ['Faculty header row not found'], duplicates: 0 };

      const slNoIndex = headers.findIndex(h => h && h.toString().includes('SL NO'));
      const staffIdIndex = headers.findIndex(h => h && (h.toString().includes('Staff ID') || h.toString().includes('Employee ID')));
      const nameIndex = headers.findIndex(h => h && h.toString().includes('Name'));
      const emailIndex = headers.findIndex(h => h && (h.toString().includes('Email') || h.toString().includes('E-mail')));
      const mobileIndex = headers.findIndex(h => h && h.toString().includes('Mobile'));
      const deptIndex = headers.findIndex(h => h && (h.toString().includes('Dept') || h.toString().includes('Department')));
...
        if (!email || email === '') email = `${name.toString().toLowerCase().replace(/[^a-z]/g, '').substring(0, 5)}${staffId}@nsut.ac.in`;
        const cleanEmail = email.toString().toLowerCase().trim();
...
      // **Bulk Hash Faculty Passwords**
      for (let user of facultyToCreate) {
        if (user.password) user.password = await bcrypt.hash(user.password, 12);
      }
...
    } catch (error) {
      return { success: 0, errors: [`Faculty Excel processing failed: ${error.message}`], duplicates: 0 };
    }
  }
}
