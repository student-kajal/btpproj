...
// import bcrypt from 'bcryptjs';  // <-- Top pe add karo
...
// CREATE ADMIN (ALWAYS HASHES PASSWORD)
// export const createAdminUser = async () => {
//   try {
//     const adminExists = await User.findOne({ role: 'admin' });
//     if (!adminExists) {
...
export const getDashboard = async (req, res) => {
  try {
    ...
    const recentGroups = await Group.find().populate('members', 'name').populate('professor', 'name').sort({ createdAt: -1 }).limit(5);

    res.status(200).json({
      success: true,
      data: {
        stats: { totalUsers, totalStudents, totalProfessors, totalGroups, activeGroups, totalProjects },
        recent: { users: recentUsers, groups: recentGroups }
      }
    });
  } catch (error) {
    console.error('Admin dashboard error:', error);
...
export const uploadStudents = async (req, res) => {
  try {
    ...
    const results = await ExcelService.processStudentExcel(req.file.buffer, session, semester);
    res.status(200).json({ success: true, message: 'Students uploaded successfully', data: results });
  } catch (error) {
    console.error('Upload students error:', error);
...
export const uploadFaculty = async (req, res) => {
  try {
    ...
    const results = await ExcelService.processFacultyExcel(req.file.buffer, session, semester);
    res.status(200).json({ success: true, message: 'Faculty uploaded successfully', data: results });
  } catch (error) {
    console.error('Upload faculty error:', error);
    res.status(500).json({ success: false, message: 'Failed to upload faculty' });
  }
};
...
export const getAllUsers = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      data: { users, totalPages: Math.ceil(total / limit), currentPage: page, total }
    });
  } catch (error) {
    console.error('Get all users error:', error);
...
export const getAnalytics = async (req, res) => {
  try {
    ...
    const thirtyDaysAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
    const recentStudents = await User.find({ role: 'student', createdAt: { $gte: thirtyDaysAgo } })
      .sort({ createdAt: -1 }).limit(10);
    const recentProfessors = await User.find({ role: 'professor', createdAt: { $gte: thirtyDaysAgo } })
      .sort({ createdAt: -1 }).limit(10);
    const skip = (page - 1) * limit;
    let sortQuery = {};
    if (sortBy === 'userId') sortQuery = { userId: 1 };
    else if (sortBy === 'name') sortQuery = { name: 1 };
    else sortQuery = { createdAt: -1 };
    const paginatedUsers = await User.find({ session, semester })
      .select('-password').sort(sortQuery).skip(skip).limit(parseInt(limit));
    const totalUsers = allUsers.length;
    const totalPages = Math.ceil(totalUsers / limit);
    res.json({
      success: true,
      data: {
        ...
        departmentStats,
        categoryStats,
        recentUploads: { students: recentStudents, professors: recentProfessors },
