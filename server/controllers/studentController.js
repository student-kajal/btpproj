...
// @route   GET /api/student/overview
export const getOverview = async (req, res) => {
  try {
    ...
    const stats = {
      hasGroup: !!group,
      groupMembers: group ? group.members.length : 0,
      projectAssigned: group && group.project && group.project.title ? true : false,
      publications: 0
    };

    // Count publications if group exists
    if (group) {
      const pubCount = await Publication.countDocuments({ group: group._id });
      stats.publications = pubCount;
    }
...
// @desc    Propose a new group
// @route   POST /api/student/propose-group
export const proposeGroup = async (req, res) => {
  try {
    const { groupName, members } = req.body;
    const studentId = req.user._id;

    console.log('Propose Group Request:', { groupName, members, studentId });
    ...
    console.log('Found Member Users:', memberUsers.length);

    if (memberUsers.length !== validMembers.length) {
      return res.status(400).json({
        success: false,
        message: 'One or more student IDs are invalid or not found'
      });
    }

    // Check if any member is already in a group
    const memberIds = memberUsers.map(u => u._id);
    const memberInGroup = await Group.findOne({
    ...
    await newGroup.populate('members', 'name userId email');

    console.log('Group Created Successfully:', newGroup._id);

    res.status(201).json({
      success: true,
      message: 'Group proposed successfully! Waiting for professor approval.',
      data: newGroup
    });
  } catch (error) {
    console.error('Propose Group Error:', error);
...
// @route   GET /api/student/my-group
export const getMyGroup = async (req, res) => {
  try {
    const studentId = req.user._id;
    ...
    const group = await Group.findOne({
      ...
      .populate('members', 'name userId email department')
...
export const getMyProject = async (req, res) => {
  try {
    ...
    if (!group.project || !group.project.title) {
      return res.status(404).json({
        success: false,
    ...
    const publications = await Publication.find({ group: group._id })
      .select('title doi authors publishedIn publicationDate url')
...
// @desc    Get publications for student's group
